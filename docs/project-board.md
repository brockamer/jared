...
- `model-guidance: enabled` — controls whether `jared close` enforces the wrap-time audit-emission rail. Values: `enabled` (default), `disabled`. Used by `jared close`. The config key name is historical — it originally controlled the file-time `## Model & execution guidance` body section (cut in #228); it now controls only the wrap-audit rail.
