...
- **Every phase live-verifies on the `BOARDID1` test board before merge.** Verification is
...
> must live-verify its **writes** on `BOARDID1` before merge, not trust the fake.

## 4. Live probe findings (read-only, run 2026-06-11 on `BOARDID1`)
...
- **Phase 1 — Events + closed-state.**
  ...
  - **Retention sub-probe** + per-task `createdAt`/activity reconstruction: decide whether
    **VELOCITY_TIMESTAMPS flips** or stays omitted-with-refined-note. Do **not** assume the
    flip.
  - Live-verify on `BOARDID1` (reads *and* any new write paths).
...
3. `recently_closed()` returns real data on KanbanFlow, **live-verified on `BOARDID1`**. (The
...
8. Each phase's writes are live-verified on `BOARDID1` before merge.
...
- **Events endpoint tier-gating** — docs silent on whether `/board/events` is premium-only;
  confirm on `BOARDID1` (it returned 200 there, so at minimum the current token has it).
...
## 12. Phase 1a live-verify findings (2026-06-11, read-only on `BOARDID1`)
