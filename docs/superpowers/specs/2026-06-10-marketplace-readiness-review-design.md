...
| **Backends** | **Both, live.** GitHub Projects v2 against `brockamer/jared-testbed`; KanbanFlow against the live "Jared Test" board (`BOARDID1`). Includes the capability-degradation matrix. |
...
| **Clean room** | Prefer an ephemeral container on `docker-host.example` (pinned fresh Python); fall back to a `lxc-host.example` LXC; env-scrubbed temp clone on this VM is the always-available backstop. |
...
Code harness operations**, not shell commands; an SSH shell on `docker-host.example` is a bash prompt,
...
- **Install mechanics → clean room (shell, over SSH).** Fresh `git clone`; the `jared` CLI run
  cold under a pinned fresh Python with **no `GH_TOKEN`, no `~/.env.token`, clean PATH, no
...
- Stand up the clean room: confirm SSH auth to `docker-host.example` (operator seeds `known_hosts`),
  spin an ephemeral container with a pinned Python; fall back to `lxc-host.example` LXC, then
...
- **(f) Stranger-onboarding** — do README → getting-started carry a newcomer from install →
  ...
  findajob / trailscribe, `bake-sites.md`, `~/.env.token`, the PAT quirk).
...
- **Clean-room auth:** first SSH to `docker-host.example`/`lxc-host.example` needs operator-seeded
  `known_hosts`/keys — a Phase-0 gate, not a blocker.
- **KanbanFlow live cost:** doubles the live surface and depends on the `~/.env.token` token + the
