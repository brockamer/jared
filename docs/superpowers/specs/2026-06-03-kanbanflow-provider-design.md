...
| `set_field(ref, field, value)` | resolve field → `customFieldId`, value → option `text`; `client.set_task_custom_field(id, customFieldId, value)`. |
| `move(ref, status)` | resolve status → `columnId`; `client.update_task(id, column_id=…)`. |
