...
the Phase 2/3 client reads the operator's live board `Jared Test` / `BOARDID1`
...
- Run bootstrap against `BOARDID1`, complete the interview, write the doc.
...
- **Cleanup:** delete the test task afterward; note the `cache_dir/kf-index-BOARDID1.json`
  index file is created outside the repo (no commit).
- **Worktree caveat:** run the **worktree-resident** `jared` script
  (`/home/user/Code/jared-317/skills/jared/scripts/jared`), not the main-repo one —
