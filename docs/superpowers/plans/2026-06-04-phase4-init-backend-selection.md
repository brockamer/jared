...
- Backend: kanbanflow
- Board URL: https://kanbanflow.com/board/BOARDID1
- Board ID: BOARDID1
...
def test_parse_kanbanflow_doc(tmp_path: Path) -> None:
    ...
    assert board.board_id == "BOARDID1"
...
def test_render_kanbanflow_doc_shape() -> None:
    ...
    assert "- Board ID: BOARDID1" in doc
...
**Files:** none (verification + board op). Uses the live board `BOARDID1` and `KANBANFLOW_API_TOKEN`.
...
```bash
KANBANFLOW_API_TOKEN=<token> /home/user/Code/jared-317/skills/jared/scripts/bootstrap-project.py \
...
```bash
KANBANFLOW_API_TOKEN=<token> /home/user/Code/jared-317/skills/jared/scripts/jared \
...
Delete the throwaway task from the live board (KanbanFlow UI or `jared close` then UI-delete). Confirm no `/tmp/kf-board.md` or `cache_dir/kf-index-BOARDID1.json` got committed (the index lives outside the repo by design).
...
```bash
/home/user/Code/jared-317/skills/jared/scripts/jared set 317 ... # or jared comment / gh issue edit to add ## Acceptance criteria
```
...
```bash
git -C /home/user/Code/jared-317 push -u origin feature/317-add-init-time-backend-selection-phase-4
