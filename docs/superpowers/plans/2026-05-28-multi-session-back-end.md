...
Run: `grep -n "next-session-prompt\|next_session_prompt" /home/user/Code/jared/skills/jared/scripts/jared | head -20`
...
Append to `/home/user/Code/jared/tests/test_cmd_next_session_prompt.py`:
...
Run: `cd /home/user/Code/jared && pytest tests/test_cmd_next_session_prompt.py::test_next_session_prompt_session_flag_filters_up_next -v`
...
In `/home/user/Code/jared/skills/jared/scripts/jared`, find the `next-session-prompt` subparser block (search for `"next-session-prompt"`). Add a `--session` argument alongside `--include-session-checks` and `--fresh`:
...
Run: `grep -n "Top of Up Next\|up_next\|_render_up_next" /home/user/Code/jared/skills/jared/scripts/jared | head -20`
...
In `/home/user/Code/jared/skills/jared/scripts/jared`, in the function handling `next-session-prompt`, after fetching the Up Next items but before the top-3 cap is applied, filter by `session-N` label when the flag is set. Pattern (adapt to actual variable names in the codebase):
...
Run: `cd /home/user/Code/jared && pytest tests/test_cmd_next_session_prompt.py -v`
...
Run: `cd /home/user/Code/jared && pytest tests/test_cmd_next_session_prompt.py::test_next_session_prompt_session_flag_with_no_matches_renders_empty_marker -v`
...
Run: `grep -n "next-session-prompt" /home/user/Code/jared/commands/jared-start.md`
...
In `/home/user/Code/jared/commands/jared-start.md`, locate step 1 of the flow. Update the bash invocation to:
...
Run: `cd /home/user/Code/jared && pytest`

Expected: full suite passes.

- [ ] **Step 4: Lint and type-check**

Run: `cd /home/user/Code/jared && ruff check . && ruff format --check . && mypy`
...
Open `/home/user/Code/jared/skills/jared/scripts/lib/ties.py` and apply these renames everywhere they appear in the file:
...
The docstring on `file_paths_in_body` (previously `_file_paths_in_body`) currently says it's a private helper. Open `/home/user/Code/jared/skills/jared/scripts/lib/ties.py`, find the function, and replace its docstring:
...
Run: `cd /home/user/Code/jared && pytest tests/test_ties_analyzers.py tests/test_ties_combine.py tests/test_ties_dataclasses.py tests/test_ties_format.py tests/test_cmd_ties.py -v`
...
Run: `cd /home/user/Code/jared && ruff check . && mypy`
...
Open `/home/user/Code/jared/tests/test_ties_analyzers.py`. Append at the bottom:
...
Run: `cd /home/user/Code/jared && pytest tests/test_ties_analyzers.py::test_public_extractors_importable -v`
...
Create `/home/user/Code/jared/tests/test_partition.py`:
...
Run: `cd /home/user/Code/jared && pytest tests/test_partition.py -v`

Expected: `ModuleNotFoundError: No module named 'skills.jared.scripts.lib.partition'`.

- [ ] **Step 3: Create the partition module**

Create `/home/user/Code/jared/skills/jared/scripts/lib/partition.py`:
...
Append to `/home/user/Code/jared/tests/test_partition.py`:
...
Append to `/home/user/Code/jared/skills/jared/scripts/lib/partition.py`:
...
Run: `cd /home/user/Code/jared && pytest tests/test_partition.py::test_propose_partition_disjoint_clusters_split_cleanly -v`
...
Create `/home/user/Code/jared/tests/test_cmd_propose_partition.py`:
...
Run: `cd /home/user/Code/jared && pytest tests/test_cmd_propose_partition.py -v`

Expected: argparse error — subcommand `propose-partition` not registered.

- [ ] **Step 3: Register the subcommand**

In `/home/user/Code/jared/skills/jared/scripts/jared`, find the subparser registration block (search for `add_parser`). Add a new subparser modeled on `next-session-prompt`:
...
```bash
grep -A 5 "def fetch_open_issues_for_ties" /home/user/Code/jared/skills/jared/scripts/lib/board.py
```

Update the handler line in `/home/user/Code/jared/skills/jared/scripts/jared`:
...
Run: `head -80 /home/user/Code/jared/commands/jared-stage.md`
...
In `/home/user/Code/jared/commands/jared-stage.md`, add a new section after the existing promotion proposal block (or wherever fits the existing structure). Suggested content:
...
Create `/home/user/Code/jared/tests/test_wrap_state.py`:
...
Run: `cd /home/user/Code/jared && pytest tests/test_wrap_state.py -v`

Expected: `ModuleNotFoundError: No module named 'skills.jared.scripts.lib.wrap_state'`.

- [ ] **Step 3: Create the module**

Create `/home/user/Code/jared/skills/jared/scripts/lib/wrap_state.py`:
...
Create `/home/user/Code/jared/tests/test_cmd_wrap_state.py`:
...
Run: `cd /home/user/Code/jared && pytest tests/test_cmd_wrap_state.py -v`

Expected: argparse error — `wrap-state` subcommand not registered.

- [ ] **Step 3: Register and implement the subcommand**

In `/home/user/Code/jared/skills/jared/scripts/jared`, add a subparser:
...
Run: `awk '/^5\. /,/^6\. /' /home/user/Code/jared/commands/jared-wrap.md | head -60`
...
In `/home/user/Code/jared/commands/jared-wrap.md`, after the Session-note application steps (step 5 currently) and before the worktree-removal block, add a new step block. Exact insertion: between the `## Current state` capture step and the `Clear this session's presence lock` step.
...
Run: `cat /home/user/Code/jared/skills/jared/references/parallel-sessions.md`
...
In `/home/user/Code/jared/skills/jared/references/parallel-sessions.md`, find the section titled `## Pre-parallel-session operator ritual` and replace its body. Old content (operator does `gh issue edit` by hand) becomes:
...
Run: `cat /home/user/Code/jared/skills/jared/references/parallel-sessions.md | head -100`
