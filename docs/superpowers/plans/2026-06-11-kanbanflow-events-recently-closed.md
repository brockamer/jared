...
def get_board_events(
    ...
    *,
    ...
    `{eventsLimited, events}` envelope shape is confirmed live on BOARDID1.
    """
    out: list[KfEvent] = []
    window_to = to_ts
...
### Task 4: Live verification on board `BOARDID1` (verification folded into the phase)

This is a verification task, not TDD. It satisfies the spec's "each phase live-verifies on BOARDID1 before merge" and records the **event-retention** and **`from`/`to` format** observations that Phase 1c needs.

- [ ] **Step 1: Confirm `recently_closed` returns real data via the real client**

Run (token from `~/.env.token`, never printed):

```bash
source /home/user/.env.token && python3 -c "
...
Run: `source /home/user/.env.token && /home/user/Code/jared/skills/jared/scripts/jared next-session-prompt` **against a KanbanFlow-backed `docs/project-board.md`** (the test board), and confirm the "Recently closed" section is now populated rather than empty.
...
git commit -m "docs(jared): record Phase 1a live-verify findings on BOARDID1 (Phase 1a.4)"
```
...
- "live-verify on BOARDID1" → Task 4. ✓
