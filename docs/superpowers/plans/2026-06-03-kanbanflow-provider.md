...
All work happens in the worktree `/home/user/Code/jared-316` (branch `feature/316-implement-kanbanflowprovider-phase-3`). The worktree has **no `.venv` of its own** — use the main repo's venv binaries but run from the worktree root so `pythonpath = ["."]` resolves to the worktree's code:

```bash
cd /home/user/Code/jared-316
VENV=/home/user/Code/jared/.venv/bin
# tests:  $VENV/python -m pytest <args>
# lint:   $VENV/ruff check . && $VENV/ruff format .
# types:  $VENV/mypy
```

Every `Run:` command below assumes `cd /home/user/Code/jared-316` first and `$VENV` as above.
