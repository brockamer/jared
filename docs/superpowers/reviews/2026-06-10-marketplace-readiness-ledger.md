...
| KanbanFlow (live board `BOARDID1`) | `test_kanbanflow_live.py` (2 tests) | **PASS** (run with `KANBANFLOW_API_TOKEN`, `GH_TOKEN` unset) |

### Clean room
- **Host:** `docker-host.example` — SSH key-auth OK, Docker 29.5.2. (`lxc-host.example` LXC fallback unavailable: key not authorized — **not needed**, docker-host.example is sufficient.)
