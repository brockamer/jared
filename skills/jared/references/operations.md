...
**Target-structure validation** runs before any write (applies only to KanbanFlow targets, which cannot create columns/options/swimlanes via the API): every distinct Status + Priority pair across source items is probed via `validate_fields()`; every distinct source milestone name is checked against the target's swimlane list. All missing elements are printed together; the command exits 1 if any are absent.
...
**KanbanFlow write-call quota guidance.** KanbanFlow's free tier allows approximately 1,000 API requests per hour. The dry-run report prints an upper-bound estimate: `1 create + 1 Priority POST + N extra-field POSTs` per item, plus 1 label POST per blocked-by edge and 1 POST per comment. For a board with many items or dense comments, check this estimate before running `--apply`; a KF-rate-limited run is safe to resume via `--out` after the quota resets.
